...
def generate_virtual_flaw(image_path, padding, fade, flaw_type, sigma, points, normalize, seed, CT_margin):
    ...
    try:
        ...
        CT_mask_image[y1-CT_margin:y2+CT_margin, :] = 1 # CT는 모재부에도 침범하는 결함을 합성할 수 있도록.
        CT_mask_image[y1-CT_margin-fade:y1-CT_margin, :] = np.repeat(np.linspace(0, 1, fade)[:, None], CT_mask_image.shape[1], axis=1)
        CT_mask_image[y2+CT_margin:y2+CT_margin+fade, :] = np.repeat(np.linspace(1, 0, fade)[:, None], CT_mask_image.shape[1], axis=1)
